...
const PROGRAM_MARKERS = ["standing orders", "git show origin/main:", /30[- ]minute/, "status message"];
...
for (const pr of prSections) {
	...
	if (live) {
		if (!LANES.every((m) => live.rest.includes(m))) fail(live.n, `${pr.title}: Verify, live lacks "${LANES.join(" ... ")}"`);
