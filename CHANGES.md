...
Content from the resync, 50 files written clean under the new policy. Two new principle leaves, `principle-attack-the-premise` and `principle-test-behavior-not-implementation`, land with matching index lines in poteto-mode, which also gains the evidence-or-label reply bullet. `how` follows upstream in dropping Critique Mode; `references/critic-prompt.md` and `references/critique-rubric.md` are deleted, the `how critics` row stays in `models.json`, and the port's Platform note, `menu-description`, and stamped `## Models` section survive. `check-plan.mjs` arrives adapted for the port: the program marker is standing orders rather than Cursor's `/goal`, the lane check matches "Ten lanes on" plus "at the PR head" without pinning a model slug, and `multi-phase-plan.md` points its repo paths at `plugins/pstack/` and its control surfaces at the `run` and `verify` built-ins. The autopilot playbooks drop gendered operator pronouns. The skill tree grows from 52 to 54 directories; `make-bot-ui` is deliberately not ported (see README). `models.json` is untouched.
