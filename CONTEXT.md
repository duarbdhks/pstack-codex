...
- **Panel** — the three-model diverse panel the multi-model skills (`arena`, `architect`, `interrogate`, `how`) run by default. One fact in `models.json` (`panel`); the generator stamps every copy and fails on strays. The shipped panel mixes Codex and Grok.
